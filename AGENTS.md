...
- `Assets/_CarryOn` is reserved for the future second game and should be treated as a placeholder until its requirements exist.
...
- Keep CarryOn work inside `Assets/_CarryOn` and avoid inventing gameplay or data contracts before that game is specified.
...
- Do not describe `_CarryOn` as implemented unless code actually exists.
